...
export function setupAutoAnimateDirective(app: App) {
  app.directive('AutoAnimate', vAutoAnimate)
}
...
const rotateXControls = (
  ...
  switch (action) {
    ...
    case 'remain':
      ...
      const start: CSSProperties = {
        transform: `translate(${deltaX}px, ${deltaY}px) rotateX(0)`,
      }
      const mid: CSSProperties =
        deltaX === 0 && deltaY === 0
...
const rotateYControls = (
  ...
  switch (action) {
    ...
    case 'remain':
      ...
      const start: CSSProperties = {
        transform: `translate(${deltaX}px, ${deltaY}px) rotateY(0)`,
      }
      const mid: CSSProperties =
        deltaX === 0 && deltaY === 0
          ? { transform: `translate(0,0)` }
...
export const autoAnimationPlugins: Recordable<AutoAnimationPlugin> = {
