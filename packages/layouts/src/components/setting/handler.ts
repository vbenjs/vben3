...
export function baseHandler(event: HandlerSettingEnum, value: any) {
  ...
  const config = handler(event, value, {appStore, rootSetting});
...
export function handler(event: HandlerSettingEnum, value: any, { appStore, rootSetting }): DeepPartial<ProjectConfig> {

  const { getThemeColor, getDarkMode } = rootSetting;
  switch (event) {
